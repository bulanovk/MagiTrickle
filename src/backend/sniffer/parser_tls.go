		...
		if extension == 0x00 { // server_name
			...
			for len(d) > 0 {
				...
				if nameType == 0 {
					...
					// RFC 6066 §3 forbids a trailing dot; xray
					// rejects and so do we. A spec-violating peer
					// would otherwise poison the ipset cache with
					// a domain no rule engine matches.
