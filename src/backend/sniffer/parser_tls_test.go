...
func buildClientHello(tb testing.TB, domain string, opts ...clientHelloOpt) []byte {
	...
	if cfg.padding > 0 {
		// TLS padding extension (RFC 7685, type 0x0015). Browsers
		// emit this to push ClientHellos past path MTU so middleboxes
		// can't fingerprint them by record size — useful for the
		// stitched test which needs a payload > MTU.
		padBody := make([]byte, cfg.padding)
		padExt := uint16Bytes(0x0015)
		padExt = append(padExt, uint16Bytes(uint16(len(padBody)))...)
		padExt = append(padExt, padBody...)
		extBuf = append(extBuf, padExt...)
	}
...
func withPadding(n int) clientHelloOpt {
	return func(c *clientHelloConfig) { c.padding = n }
}
...
func TestParseTLS_TrailingDotRejected(t *testing.T) {
	// RFC 6066 §3 forbids a trailing dot in the SNI. xray rejects it;
	// we now do too. A spec-violating peer would otherwise poison the
	// ipset cache with a domain no rule engine matches.
	payload := buildClientHello(t, "example.com.")
	cfg := Config{EnableTLS: true}
	got := Parse(payload, cfg)
	if got.Hit {
		t.Fatalf("trailing-dot SNI produced hit: %+v", got)
	}
}

func TestParseTLS_StitchedClientHello(t *testing.T) {
	// Real-world ClientHellos exceed MTU/MSS once ECH + ALPN +
	// supported_versions + key_share extensions stack up. The NFQUEUE
	// recv loop re-invokes Parse on the concatenated buffer via
	// parseFlowBufferShared (nfqueue_common.go); this test mirrors
	// that path at the unit level so a future regression in the
	// stitcher surfaces immediately.
	payload := buildClientHello(t, "www.cloudflare.com", withPadding(1500))
	if len(payload) < 800 {
		t.Fatalf("synthetic ClientHello too small (%d bytes); pick a longer domain", len(payload))
	}

	// Split somewhere in the middle of the handshake body, well past
	// the record header so the first segment is unambiguously a TLS
	// record but truncated.
	const splitAt = 700
	if splitAt >= len(payload) {
		t.Fatalf("split offset %d >= payload length %d", splitAt, len(payload))
	}
	first := payload[:splitAt]
	second := payload[splitAt:]

	cfg := Config{EnableTLS: true}

	// First half: parser sees a truncated ClientHello and must miss.
	if got := Parse(first, cfg); got.Hit {
		t.Fatalf("truncated first half produced hit: %+v", got)
	}

	// Stitch and re-parse — this is exactly what parseFlowBufferShared
	// does on the second TCP segment.
	stitched := append(append([]byte(nil), first...), second...)
	got := Parse(stitched, cfg)
	if !got.Hit {
		t.Fatalf("stitched parse missed: %+v", got)
	}
	if got.Domain != "www.cloudflare.com" {
		t.Errorf("Domain = %q, want www.cloudflare.com", got.Domain)
	}
	if got.Protocol != ProtocolTLS {
		t.Errorf("Protocol = %q, want tls", got.Protocol)
	}
