...
//
// resolver may be nil, in which case the system resolver (/etc/resolv.conf)
// is used. Pass a custom *net.Resolver to force DNS through a specific
// upstream — useful for stress tests that need to bypass the router's
// DNS-MITM proxy so SNI sniffing is the path that populates the ipset.
func runWorker(
	domains []string,
