...
func main() {
	...
	dnsOverride := flag.String("dns", "", "force DNS resolver to this host:port (e.g. 77.88.8.8:53); empty = use system resolver from /etc/resolv.conf")
	flag.Parse()
	...
	// Build resolver. nil = use system resolver.
	resolver := buildResolver(*dnsOverride)
	if resolver != nil {
		log.Printf("using DNS override: %s", *dnsOverride)
	}
	...
	launchWorker := func() {
		...
		go func() {
			...
			runWorker(domains, 5*time.Second, 5*time.Second, resolver, results, &failed, stop)
		}()
	}
...
// buildResolver returns a *net.Resolver that sends all DNS queries to the
// given host:port (UDP), or nil if addr is empty (= use system resolver).
// PreferGo forces Go's pure-Go resolver so cgo isn't required and so the
// override is independent of libc's resolver state.
//
// Use this to bypass the router's DNS-MITM proxy during stress tests:
// forcing an external resolver means the router never learns the IPs via
// DNS, so SNI sniffing is the only path that populates the routing ipset.
// This is the realistic scenario for traffic that hard-codes a public
// resolver (DoH fallback, hard-coded 8.8.8.8, etc.).
func buildResolver(addr string) *net.Resolver {
	if addr == "" {
		return nil
	}
	// Accept "1.2.3.4" or "1.2.3.4:53"; normalize to host:port.
	host := addr
